...
function ayuda(){
	...
	echo -e "\t${morado}-c${finColor}  Modo Revision Completa"
...
function revision_errores(){
	...
	#echo -en "${turquesa}[+]${finColor} Introduce Ultima IP: " && read ip_final
...
#------------------
	...
	for descarga in $(seq 16 $ip_final); do
		...
		nombres="$(curl http://$rango.$descarga/ISAPI/System/deviceInfo --digest -u "admin:$password" -s | grep "<deviceName>" | cut -c 13-16)"
		...
		sd_pet="$(curl http://$rango.$descarga/ISAPI/ContentMgmt/Storage/hdd --digest -u "admin:$password" -s | grep "<status>" -A 1 | sed 's/<status>/Formateo:/' | sed 's/<capacity>/Capacidad:/' | awk -F "<" '{print $1}' | xargs -n 1 | sponge -a tmp/$descarga.txt)"
...
#------------------------------------
	...
	if [[ $ip_final -le 35 ]]; then
		...
		grabador_1_1="$(ping -c 1 $rango.11 > /dev/null && echo "${turquesa}[+]${finColor} ${blanco}NVR1 -${finColor} ${verde}ONLINE${finColor}" || echo "${amarillo}[!]${finColor} ${blanco}NVR1 -${finColor} ${rojo}OFFLINE${finColor}")"
		echo -e "\n$grabador_1_1\n"

		for dispositivos_nvr1_1 in $(seq 16 $ip_final); do

			barrido_nvr1_1="$(ping -c 1 $rango.$dispositivos_nvr1_1 >/dev/null && echo -e "${turquesa}[+]${finColor} ${blanco}Camara $dispositivos_nvr1_1 -${finColor} ${verde}ONLINE${finColor}" || echo -e "${rojo}[!]${finColor} ${blanco}Camara $dispositivos_nvr1_1 -${finColor} ${rojo}OFFLINE${finColor}")"
	...
	if [[ $ip_final -ge 36 || $ip_final -eq 55 && $ip_final -lt 56 ]]; then
		...
		for dispositivos_nvr1_2 in {16..35}; do
			barrido_nvr1_2="$(ping -c 1 $rango.$dispositivos_nvr1_2)"
		done

		echo "ping -c 1 $rango.12"
		for dispositivos_nvr2_2 in $(seq 36 $ip_final); do
			barrido_nvr2_2="$(ping -c 1 $rango.$dispositivos_nvr2_2)"
		done
	fi
...
while getopts "crh" arg; do
	case $arg in
		c) revision_completa; let contador_argumentos+=1;;
