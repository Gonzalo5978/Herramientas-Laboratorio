...
for camaras in $(seq $camaraInicio $camaraFinal); do
	...
	red="$(curl http://$rango.$camaras/ISAPI/System/Network/interfaces --digest -u "admin:$password" -s | head -n 20 | tail -n 8 | grep "<ipAddress>" | sed '2,3 s/<ipAddress>/DNS:/' | sed '1 s/<ipAddress>/Gateway:/' | awk -F "<" '{print $1}')"
